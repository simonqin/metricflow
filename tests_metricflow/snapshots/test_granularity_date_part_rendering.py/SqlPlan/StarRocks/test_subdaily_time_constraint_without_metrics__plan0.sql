      ...
      FROM (
        ...
        FROM (
          ...
          SELECT /*+ SET_VAR(cte_inline=true) */
            ...
            , YEAR(time_spine_src_28003.ts) AS ts__extract_year
            , QUARTER(time_spine_src_28003.ts) AS ts__extract_quarter
            , MONTH(time_spine_src_28003.ts) AS ts__extract_month
            , DAY(time_spine_src_28003.ts) AS ts__extract_day
            , DAY_OF_WEEK_ISO(time_spine_src_28003.ts) AS ts__extract_dow
