...
FROM (
  ...
  FROM (
    ...
    FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              SELECT /*+ SET_VAR(cte_inline=true) */
                ...
                , YEAR(time_spine_src_28005.ts) AS ts__extract_year
                , QUARTER(time_spine_src_28005.ts) AS ts__extract_quarter
                , MONTH(time_spine_src_28005.ts) AS ts__extract_month
                , DAY(time_spine_src_28005.ts) AS ts__extract_day
                , DAY_OF_WEEK_ISO(time_spine_src_28005.ts) AS ts__extract_dow
                , DAYOFYEAR(time_spine_src_28005.ts) AS ts__extract_doy
      ...
      ON
        DATE_SUB(subq_8.metric_time__hour, INTERVAL 1 HOUR) = subq_4.metric_time__hour
    ) subq_9
  ) subq_10
) subq_11
